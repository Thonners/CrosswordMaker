...
public class Crossword {
    ...
    public static final String CROSSWORD_EXTRA_TITLE = "com.thonners.crosswordmaker" +
            ".crossword_title";
    public static final String CROSSWORD_EXTRA_DATE = "com.thonners.crosswordmaker.crossword_date";
    public static final String CROSSWORD_EXTRA_NO_ROWS = "com.thonners.crosswordmaker" +
            ".crossword_no_rows";
    ...
    public static final int SAVE_ARRAY_START_INDEX = 6;       // Update this if format of
    // saveArray changes, i.e. if more fields are added before the grid is saved.

    private Context context;

    public String title = "Default title";   // Add the displayDate to the default title?
    public String date;    // Save displayDate format (yyyyMMdd)          //= getFormattedDate()
    // ;       // Default displayDate
    ...
    private final int borderWidth = 1;   // This is the width of the border around the cells.
    // (Needs to be accounted for in cell width calculation else final column is too thin
    ...
    private final ArrayList<Clue> hClues = new ArrayList<>();     // Storage for all the
    // horizontal clues
    private final ArrayList<Clue> vClues = new ArrayList<>();     // Storage for all the vertical
    // clues
    ...
    public Crossword(Context context, CrosswordGrid crosswordGrid, String[] savedCrossword,
                     boolean editMode) {
        // Constructor for a saved crossword
        this.context = context;
    ...
    private void createGrid() {
        ...
        grid.setBackground(ResourcesCompat.getDrawable(context.getResources(),
                R.drawable.cell_white, null));
    }
    ...
    private void fillCells() {
        ...
        for (int i = 0; i < rowCount; i++) {
            for (int j = 0; j < rowCount; j++) {
                ...
                //                Log.d(LOG_TAG,"For cell " + cells[i][j].getCellName() + ", read
                //                value: " + tempString);
                if (tempString.matches("-")) {
                    //                    Log.d(LOG_TAG,"Value matched '-', so toggling black
                    //                    cell.");
    ...
    public void findClues() {
        ...
        // Print cells in first clue to check for duplicates. Check size > 0 to prevent it
        // crashing due to IndexOutOfBounds Exception
        if (hClues.size() > 0) {
    ...
    private void findHorizontalClues(int row) {
        ...
        for (int col = 0; col < rowCount; col++) {  // only go as far as rowCount-1 because there
            // cannot be a horizontal clue in the final column.
            ...
            if (col < rowCount - 1 && nextWhiteCellNewClue && !cells[row][col].isBlackCell() && !cells[row][col + 1].isBlackCell()) {
                ...
                hClues.add(newClue);                                            // Add clue to
                // collection of clues
                hClues.get(horizontalClueIndex).addCellToClue(cells[row][col]);     // Add cell
                // to list of cells in clue
                cells[row][col].setHClue(hClues.get(horizontalClueIndex));          // Tell the
                // cell which clue it belongs to

                Log.d("Clues", "New clue found at: r = " + row + " & c = " + col);
                Log.d("Clues", "horizontalClueIndex = " + horizontalClueIndex + " & hClues.length" +
                        " = " + hClues.size());

            } else if (!nextWhiteCellNewClue && !cells[row][col].isBlackCell()) {
                // If white cell, augment clue count. If not, reset nextWhiteCellNewClue
                int clueLength =
                        col - hClues.get(horizontalClueIndex).getStartCell().getColumn() + 1;
    ...
    private void findVerticalClues(int col) {
        ...
        for (int row = 0; row < rowCount; row++) {
            ...
            // only go as far as rowCount-1 because there cannot be a vertical clue starting in
            // the final row.
            ...
            if (row < rowCount - 1 && nextWhiteCellNewClue && !cells[row][col].isBlackCell() && !cells[row + 1][col].isBlackCell()) {
                ...
                vClues.add(newClue);                                            // Add clue to
                // collection of clues
                vClues.get(verticalClueIndex).addCellToClue(cells[row][col]);     // Add cell to
                // list of cells in clue
                cells[row][col].setVClue(vClues.get(verticalClueIndex));          // Tell the
                // cell which clue it belongs to

                Log.d("Clues", "New clue found at: r = " + row + " & c = " + col);
                Log.d("Clues",
                        "verticalClueIndex = " + verticalClueIndex + " & vClues.length = " + vClues.size());

            } else if (!nextWhiteCellNewClue && !cells[row][col].isBlackCell()) {
                // If white cell, augment clue count. If not, reset nextWhiteCellNewClue
                int clueLength = row - vClues.get(verticalClueIndex).getStartCell().getRow() + 1;
                vClues.get(verticalClueIndex).setLength(clueLength); // TODO: Make this length be
                // calculated from the number of cells in the clue, rather than set separately
                vClues.get(verticalClueIndex).addCellToClue(cells[row][col]);     // Add cell to
                // list of cells in clue
                cells[row][col].setVClue(vClues.get(verticalClueIndex));          // Tell the
                // cell which clue it belongs to
    ...
    private void calculateCellWidth() {
        ...
        // Make cell size (screenWidth / (rowCount + 1)). This allows half a cellWidth on each
        // side to keep the borders nice
    ...
    public void setDate(String inputDate) {
        // String inputDate to be provided from HomeActivity intent, which should put it into
        // yyyyMMdd.
        this.date = inputDate;
    }
    ...
    public String getDisplayDate() {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Couldn't parse Crossword.displayDate (should be in save format) into " +
                    "something useful. This is coming from HomeActivity via intents so check the " +
                    "routing!");
            return context.getResources().getString(R.string.error_crossword_date); // Return the
    ...
    public static String getDisplayDate(Context context1, String savedDate) {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat(SAVE_DATE_FORMAT);      // Save Formatted
        // displayDate
        DateFormat localeDateFormat = android.text.format.DateFormat.getDateFormat(context1);
        // Locale displayDate format
        Date dateProper;
        ...
        try {
            ...
            Log.e(LOG_TAG, "Couldn't parse Crossword.displayDate (should be in save format) into " +
                    "something useful. This is coming from HomeActivity via intents so check the " +
                    "routing!");
            return context1.getResources().getString(R.string.error_crossword_date); // Return
    ...
    public void cellClickedWhenHyphenWordSplitActive(Cell cellClicked) {
        ...
        if (activeCell == null) {
            ...
            // Show the instructions if we can't work out which clue to add the hyphen/word-split
            // to automatically
    ...
    public void toggleOppositeBlackCell(Cell inputCell) {
        // Makes use of crossword's rotational symmetry to speed the process of creating your own
        // grid.
        ...
        if (oppositeCol == inputCell.getColumn() && oppositeRow == inputCell.getRow()) {
            ...
            //           Log.d(LOG_TAG, "Toggling blackCell opposite: " + inputCell.getCellName()
            //           + ", i.e. toggling: " + cells[oppositeRow][oppositeCol].getCellName());
    ...
    private void getClueNumbers() {
        ...
        Integer[] lastCellCoords = {rowCount, rowCount};          // Initialise coords as last
        // cell so that it will definitely be corrected on the first run through
        ...
        // Loop through all coords and sort them into order. Once the next startCell is found,
        // remove it from the general list.
        while (startCellCoordsGeneral.size() > 0) {
            for (Integer[] coords : startCellCoordsGeneral) {
                // Make nextClueStartCell equal to that under investigation if its row is less
            ...
            if (nextClueStartCellCoords != lastCellCoords) {
                //               Log.d(LOG_TAG, "Next Clue Start Cell Coords = (" +
                //               nextClueStartCellCoords[0] + "," + nextClueStartCellCoords[1] +
                //               ").");
    ...
    public String[] getSaveArray() {
        ...
        //          array[6-n]  = crossword cells. "" denotes a blank (white) cell. "-" denotes a
        //          black cell.
        ...
        for (int i = 0; i < rowCount; i++) {
            for (int j = 0; j < rowCount; j++) {
                if (!cells[i][j].isBlackCell()) {
                    saveArray[index] = cells[i][j].getText().toString();   // Save whatever text
                    // is in the box to the saveArray. Will be empty if box is empty.
                    if (cells[i][j].hyphens[Cell.CellSide.RIGHT.ordinal()])
                        saveArray[index] += SAVE_RIGHT_HYPHEN_CODE;
                    if (cells[i][j].hyphens[Cell.CellSide.BOTTOM.ordinal()])
                        saveArray[index] += SAVE_BOTTOM_HYPHEN_CODE;
                    if (cells[i][j].wordSplits[Cell.CellSide.RIGHT.ordinal()])
                        saveArray[index] += SAVE_RIGHT_WORD_SPLIT_CODE;
                    if (cells[i][j].wordSplits[Cell.CellSide.BOTTOM.ordinal()])
                        saveArray[index] += SAVE_BOTTOM_WORD_SPLIT_CODE;
                } else {
                    saveArray[index] = "-";
                }

                //                Log.d("SAVEARRAY:", "At index = " + index + ", saveArray[index]
                //                = " + saveArray[index]);  // Don't output this at the moment
                //                because it seems to work, but spams the log thread. If saving
                //                stops working, uncomment this line.
                index++;
            }
        }

        //        for (int k = 0 ; k < saveArray.length ; k++) {
        //            Log.d(LOG_TAG, "saveArray[" + k + "] = " + saveArray[k]) ;  // Don't output
        //            this at the moment because it seems to work, but spams the log thread. If
        //            saving stops working, uncomment this line.
        //        }
    ...
    public void toggleZoom() {
        ...
        if (!isZoomedIn) {
            ...
            newCellSize =
                    context.getResources().getDimensionPixelOffset(R.dimen.cell_size_zoomed_in);
        }
...
/*                cells[i][j].setWidth(newCellSize);
    ...
    public void scrollToCell(Cell cell) {
        // Method to call scrollToCell from CrosswordGrid. This will tell the scroll view to
    ...
    /**
     * @return The horizontal (i.e. across) clues, sorted into numerical order according to their
     * display number
     */
    ...
    /**
     * @return The vertical (i.e. down) clues, sorted into numerical order according to their
     * display number
     */
    ...
    public void saveCrossword(String[] saveArray) {
        try { // TODO: Move to try with resources
    ...
    public void initialiseSaveFiles() {
        // Format File name
        fileName = date + "-" + title.replaceAll(" ", "_").replaceAll("-", "__") +
                "_deprecatedCrossword"; //.toLowerCase() ; // Delete this if it works
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            // TODO: Move this to use the MediaStore API
        } else {
            docsDirectory = new File(Environment.getExternalStorageDirectory() + "/" +
                    ".CrosswordToolkit");
